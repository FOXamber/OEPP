...
def condition_projection(x, conditions, action_dim, class_dim, horizon_dim):
    for t, val in conditions.items():
        if type(t) != str:
            x[:, t, horizon_dim + class_dim + action_dim:] = val.clone()

    x[:, 1:-1, horizon_dim + class_dim + action_dim:] = 0.
...
class Weighted_MSE(nn.Module):
    ...
    def __init__(self, weights, action_dim, horizon_dim, class_dim):
        super().__init__()
