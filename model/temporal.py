...
class ResidualTemporalBlock_wot(nn.Module):

    def __init__(self, inp_channels, out_channels, kernel_size=3):
        super().__init__()

        self.blocks = nn.ModuleList([
            Conv1dBlock(inp_channels, out_channels * 4, kernel_size),
            Conv1dBlock(out_channels * 4, out_channels, kernel_size, if_zero=True)
        ])
        self.residual_conv = nn.Conv1d(inp_channels, out_channels, 1) \
            if inp_channels != out_channels else nn.Identity()
...
class TemporalUnet(nn.Module):
    def __init__(
        ...
        # self.task_embed = ResidualTemporalBlock(18, 512, embed_dim=time_dim)
        # self.task_embed = nn.Embedding(180, 778)

    def forward(self, x, time, task):
        ...
        # k = x.shape[-1]
        # task = task.unsqueeze(2).repeat(1, 1, k)
        # task = self.task_embed(task).unsqueeze(2).repeat(1, 1, k)
